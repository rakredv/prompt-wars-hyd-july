...
export default function Dashboard({ user, demoMode, showToast }) {
  ...
  const fetchData = async () => {
    setLoadingData(true);
    try {
      const data = await getHealthData(user.uid, demoMode, 30);
      setHealthData(data);
    } catch (e) {
  ...
  const fetchInsight = async (forceRefresh = false) => {
    ...
    try {
      ...
      if (!forceRefresh) {
        const cached = await getLatestInsight(user.uid, demoMode);
        if (cached) {
      ...
      const data = await getHealthData(user.uid, demoMode, 14);
  ...
  const handleSeedDemo = async () => {
    ...
    try {
      const token = await user.getIdToken();
      const res = await fetch(`${API_BASE}/api/ai/demo-seed`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ uid: user.uid }),
      });
      if (!res.ok) throw new Error(await res.text());
      showToast('Demo data generated by AI! ✨', 'success');
      await fetchData();
    } catch (e) {
      showToast('Failed to generate demo: ' + e.message, 'error');
    } finally {
      setSeeding(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
      {/* Demo seed prompt */}
      {demoMode && !hasData && !loadingData && (
        <div style={{
          background: 'linear-gradient(135deg, rgba(124,58,237,0.1), rgba(176,64,247,0.05))',
          border: '1px solid rgba(124,58,237,0.25)', borderRadius: 'var(--radius-xl)',
          padding: 'var(--space-2xl)', textAlign: 'center',
          display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-lg)',
        }}>
          <div style={{ fontSize: '3rem' }}>🧠</div>
          <div>
            <h2 style={{ marginBottom: 8 }}>See MindYou in action</h2>
            <p style={{ maxWidth: 320, margin: '0 auto' }}>
              Let AI generate realistic health data so you can explore all features.
            </p>
          </div>
          <button id="btn-seed-demo" className="btn btn-primary" onClick={handleSeedDemo}
            disabled={seeding} style={{ padding: '12px 32px' }}>
            {seeding ? <><span className="spinner" style={{ width: 16, height: 16 }} /> Generating...</> : '✨ Generate Demo Data'}
          </button>
          <p style={{ fontSize: '0.75rem', color: 'var(--text-tertiary)' }}>Or switch to Live Mode to connect Apple Health</p>
        </div>
      )}

      {hasData && <InsightCard insight={insight} loading={loadingInsight} onRefresh={() => fetchInsight(true)} />}
